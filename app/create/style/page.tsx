...
const STYLES = [
  { id: "copywriting", label: "Copywriting vendeur", icon: "✨" },
  { id: "tiktok", label: "TikTok / dynamique", icon: "🔥" },
  { id: "pedagogique", label: "Pédagogique", icon: "🎓" },
  { id: "professionnel", label: "Professionnel", icon: "💼" },
  { id: "expert", label: "Expert / technique", icon: "🧠" },
  { id: "storytelling", label: "Storytelling narratif", icon: "📖" },
  { id: "minimaliste", label: "Simple & minimaliste", icon: "😌" },
];

export default function StylePage() {
  const router = useRouter();
  const { style, setStyle } = useEbookStore();
  ...
  function handleNext() {
    ...
    setStyle(localStyle);
    router.push("/create/length");
  }

  return (
    <main className="relative min-h-[100dvh] overflow-hidden bg-gradient-to-b from-white via-slate-50 to-white 
      ...
      <section className="max-w-3xl mx-auto text-center px-6 mt-14 animate-fade-in">
        ...
        <p className="text-gray-600 dark:text-gray-300 mt-4 text-lg max-w-2xl mx-auto">
          Ce style influencera la manière dont l&apos;IA écrira ton ebook.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-12">
          ...
          {STYLES.map((s, index) => (
            <button
              ...
              className={`
                relative p-6 rounded-2xl shadow-md backdrop-blur-xl border 
                transition-all duration-200 fade-in-up
                ${localStyle === s.id 
                  ? "bg-blue-600 text-white border-blue-700 scale-[1.04] shadow-blue-500/40" 
                  : "bg-white/70 dark:bg-gray-800/60 border-gray-300 dark:border-gray-700 hover:scale-[1.02]"
                }
              `}
